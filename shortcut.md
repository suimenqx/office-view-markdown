# Office View Markdown shortcuts

## 中文

### 通用

扩展级快捷键：`Ctrl Alt E`（macOS：`⌃⌘ E`）切换到 VS Code 文本编辑器。桌面扩展还会在 Markdown 文本编辑器中用 `Ctrl V`（macOS：`⌘ V`）增强图片粘贴。
...
Extension shortcuts: `Ctrl Alt E` (macOS: `⌃⌘ E`) switches to VS Code's text editor. In the desktop extension, `Ctrl V` (macOS: `⌘ V`) also provides enhanced image paste in the Markdown text editor.
